...
class EvalCkptDriver(object):
  ...
  def run_inference(self,
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
      ...
      prediction_idx = []
      prediction_prob = []
      for _ in tqdm(range(len(image_files) // self.batch_size)):
  ...
  def run_lite_inference(self,
      ...
      with tf.Graph().as_default(), tf.Session() as sess:
        ...
        for i in tqdm(range(len(image_files) // self.batch_size)):
          ...
          probs = probs[0]
          idx = np.argsort(probs)[::-1][:5]-label_offset
          top1_cnt += labels[i] in idx[:1]
          top5_cnt += labels[i] in idx[:5]
          if i % 100 == 0:
            print('Step {}: top1_cnt= {}  top1_acc = {:4.2f}%  top5_cnt = {} top5_acc = {:4.2f}%'.format(
            i, top1_cnt, 100 * top1_cnt / (i + 1), top5_cnt, 100 * top5_cnt / (i + 1)))
            sys.stdout.flush()
  ...
  def eval_imagenet_lite(self, tflite_path, imagenet_eval_glob,
                    imagenet_eval_label, num_images, enable_ema, export_ckpt):
    """Eval ImageNet images and report top1/top5 accuracy.
    ...
    top1_cnt, top5_cnt=self.run_lite_inference(tflite_path, image_files, labels, enable_ema, export_ckpt)
    top1, top5 = 100 * top1_cnt / num_images, 100 * top5_cnt / num_images
    print('Final: top1_acc = {:4.2f}%  top5_acc = {:4.2f}%'.format(top1, top5))
    return top1, top5
